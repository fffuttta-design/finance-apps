...
- 一覧の各カード行をタップ → **棚卸しシート**を開く（v1.0.313〜）。
  ...
  - **荒療治：CSVで置き換え（v1.0.321〜）**: 棚卸しのコストが高すぎる時の最終手段。CSV読込後に
    「CSVで置き換える」ボタンを表示。**CSV期間内のそのカードの既存取引を全削除 → CSV各行を新規取引として
    取り込み**、**科目は店名からAIが一括推定**（`StoreCategoryClassifier`・Gemini 1回呼び出し、
    現モードのカテゴリ候補から選択。失敗時は「未分類」）。**実行直前に自動バックアップ**
    （`savePreImportSnapshot`）。確認ダイアログ必須・モード別カットオフ前はスキップ。
    カード・対象期間は棚卸しシート（カード×CSV期間）で限定されるため「指定カード・指定期間だけ」を初期化できる。
