...
> **最終更新: 2026-07-03 / v1.0.418+419**
> 変更があるたびにこのファイルを編集してバージョンを更新すること。
>
> **v1.0.417-418 の主な変更（明細テーブル／支出入力）**
> - 明細の並び替えは **「カスタム順」モード** のときだけ有効（右上のトグルで切替）。
>   並べた順は各行の `sortOrder` に保存され、カスタム順で再表示される。
> - **日付ソートは純粋な日付順**（手動並び順に影響されない）に変更。
> - 行の長押しドラッグ開始を 500ms→**250ms** に短縮。
> - **チェック／並び替えが一瞬元に戻る不具合を根治**（Firestore書き込みと同時に
>   メモリキャッシュへも即反映する楽観的更新。`FirestoreTransactionRepository`）。
> - 固定費の自動実明細化が **モードの表示下限(minDate)より前を毎回作り直す不具合**を修正
>   （`FixedCostMaterializer` の生成下限を minDate に合わせた）。
> - 支出入力の日付欄：**全角数字→半角**自動変換・composing下線の除去・曜日表示の撤去。
> - 支出入力の場所欄：`RawAutocomplete` 化で **↑↓キー選択・Enter/クリック確定** に対応。
