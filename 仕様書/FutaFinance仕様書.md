...
- 一覧の各カード行をタップ → **棚卸しシート**を開く（v1.0.313〜）。
  ...
  - 差額 = 0: 「一致」表示。
  - **明細コピペ突合（v1.0.316〜）**: 棚卸しシート内「明細コピペで突合」でカード会社サイトの利用明細を
    貼り付け → 行ごとに金額を解析（`_parseStatement`）し、記録済み取引と**金額で突合**。結果を
    「一致／記録漏れ（明細にあり記録なし）／要確認（記録にあり明細なし）」に分類。記録漏れは
    店名・日付・金額をプリフィルした**[追加]ボタンでワンタップ登録**でき、差額（＝記録漏れ合計）を解消できる。
