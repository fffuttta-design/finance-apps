...
- **並び替えUIはプルダウン→バッジ（チップ）**（新しい順／古い順／高い順／安い順／**カテゴリ順**。選択中はアクセント色）（v1.0.353〜、カテゴリ順は v1.0.355〜）。
...
- **共通ウィジェット化（v1.0.360〜）**：この表（検索・並び替えバッジ・列幅ドラッグ・カテゴリ色）を `v2/widgets/expense_detail_table.dart` の `ExpenseDetailTable` に集約。支出タブ（rich_expenses）と**クレカ詳細（`CardDetailScreen` の明細タブ・PC幅）**で共用（スマホ幅のクレカ明細は従来のカード型リスト）。
