...
> **最終更新: 2026-08-06 / v1.0.558+559**
> 変更があるたびにこのファイルを編集してバージョンを更新すること。
>
> **v1.0.558 の主な変更（2026-08-06）**（支出明細に振替を表示しない）
>
> - ✨ **支出明細（共通テーブル `ExpenseDetailTable`）から振替（`TransactionType.transfer`）を除外**。
>   口座間移動・カード引落などの振替は「支出」でも「消費」でもなく、月次決算の銀行CSV取り込みで
>   別途補完されるため、明細表・支払方法別の件数集計の両方から常に除外する（`_nonTransferRows`）。
>
> **v1.0.557 の主な変更（2026-08-06）**（クレカ引落の自動生成をカード単位でOFFにできるように／請求推移の引落日を翌営業日表示に）
>
> - ✨ **カードごとに「引落を自動生成しない」トグル**（`RegisteredCreditCard.noAutoSettlement`）。ONにすると
>   `CardSettlementService` がそのカードの引落（振替）を自動生成しなくなる。**引落口座を銀行CSVでミラーして
>   いるカードはON**にする運用（引落は銀行明細から本物が入るので、見込みの自動引落を作ると二重・金額ズレの
>   原因になる）。カード編集シート（`card_editor_screen.dart`）にトグル追加。通帳ミラー方針に沿った整理で、
>   締め・確認済み撤去（v1.0.555）と同じ「手入力時代の遺物を畳む」系。
> - 🐛 **請求推移タブの引落日表示を翌営業日に補正**（`card_detail_screen.dart` の `_billingRow`）。従来は
>   「毎月27日」を生で表示していたため、引落日が土日祝でも27日のままだった。月末クランプ＋`JpHolidays.
>   nextBusinessDay` で、自動引落の生成日と同じ規則の実引落日を表示するよう修正。
>
> **v1.0.556 の主な変更（2026-08-05）**（固定費（サブスク）も消費・非消費で分けられるように）
>
> - ✨ **固定費（サブスク）にも「消費に含めない」トグル**（`Subscription.nonConsumption`）。社会保険料などを
>   固定費として登録している場合、これをONにすると支出タブで**非消費側の小計**に回る（消費合計に入らない）。
>   サブスク編集（`subscription_list_screen.dart`）にトグル追加、`rich_expenses.dart` が固定費を消費/非消費に
>   振り分け。⚠v1.0.555同様、**収支・残高・PL計算からは除外しない**（表示分離のみ）。
>
> **v1.0.555 の主な変更（2026-08-05）**（締め・確認済みを完全撤去／支出の消費・非消費2区分／業績に月×科目マトリクス）
>
> - 🗑 **「締め機能（月次締め）」と「確認済みチェック」を完全撤去**（本人決定）。CSV取り込みで各行が
>   銀行明細そのもの（残高アンカー仮IDで自動一致）になり、手で1行ずつ確認したり月を凍結する作業が
>   不要になったため。撤去対象＝`month_closing_*`／`checklist_*`／各明細の確認済みチェックUI・締めバー・
>   締め済みグレーアウト・締め確認ダイアログ。関連モデル/リポジトリ/画面/バックアップ配線も削除。
>   - `Transaction.reviewed`／`Subscription.reviewedMonths` の**フィールド定義は後方互換のため休眠保持**
>     （UIは全撤去・読み書きせず）。Firestoreの `config/{business,personal}_{month_closing,checklist}`
>     4ドキュメントは**バックアップの上で削除**（VPS `backups/futafinance-closing-checklist-removed.json`）。
>   - `card_settlement_service.dart`：締め済み月の自動引落抑止を廃止し、**「同月・同カード宛の振替が既にあれば
>     生成しない」重複防止ガード**に置換（銀行CSVミラーと自動引落の二重計上を防止）。
> - ✨ **支出の「消費／非消費」2区分**（`MajorCategory.nonConsumption`）。税金・社会保険・各種手数料など
>   「必須だが消費でない支出」を、支出タブ上で**消費支出とは別小計**に分けて表示（既定は畳む）。
>   支出タブの合計＝**消費支出のみ**を主表示、消費＋非消費の総額も薄く併記。カテゴリ編集画面に
>   「消費に含めない」トグル追加。⚠**収支（収入−支出）・残高・月次スナップショット・PLの計算からは
>   除外しない**（表示上の分離のみ。税金等も実際の出金なので計算には全額含める）。
> - ✨ **業績タブに「カテゴリ別（月×科目マトリクス）」ビュー追加**（`v2_report.dart`）。行＝科目／列＝各月
>   （事業=決算年度10〜9月／個人=暦年1〜12月）／セル＝合計、年計列・月合計行つき。支出（科目別）と
>   収入（収入源別）の2表、事業モードはセクション小計、セルタップで明細検索へドリルダウン。
>
> **v1.0.554 の主な変更（2026-08-04）**（締め機能・確認チェックを Windows/Web 限定に＝Androidアプリでは非表示。※v1.0.555で締め機能自体を撤去したため本項は歴史的記録）
>
> - ✨ **月次「締め機能」と「確認チェック」を Windows/Web のみ表示に変更**（Androidアプリでは非表示）。
>   本人の運用方針で、締めはPC（Windows）でのみ行うため、Androidでは不要なUIを消してスッキリさせた。
>   - 通帳（`account_detail_screen.dart`）：「◯月を締める」バー・「確認済み N/件」カウンタ・
>     各明細行の確認チェックボックスを、`kIsWeb`（＝Windows/Web）でのみ表示。
>   - 共通の締めバー（`month_closing_bar.dart`＝ホーム/支出/収入タブ）も同様に Android では非表示。
>   - 判定は `kIsWeb`：**Windowsデスクトップは Flutter Web を Electron で包む構成のため kIsWeb=true**、
>     Androidネイティブは kIsWeb=false。これで「PCだけ締め機能あり／スマホは無し」を実現。
>   - データ自体（reviewed/締めフラグ）は共有のまま。PCで締めた状態はAndroidにも反映される（表示のみ）。
>
> **v1.0.553 の主な変更（2026-08-04）**（「カテゴリを提案」を強化＝履歴が無ければAIで店舗・取引内容の文言から推定）
>
> - ✨ **「✨カテゴリを提案」を強化**（`expense_input_screen.dart`）。従来は**過去の履歴の完全一致**
>   だけで、初めての店では「似た履歴が見つかりませんでした」で終わっていた。**履歴に無い場合は
>   AI(Gemini)が店舗名＋取引内容の文言からカテゴリを推定**するように追加（`StoreCategoryClassifier`
>   を利用）。例:「おもて呼吸器・アレルギー内科クリニック」「アレルギー検査・結果診断」→
>   **病院・薬 › 内科** に到達できる。推定中はボタンにスピナー＋「推定中...」を表示。
>   推定結果はユーザーのカテゴリ一覧の中から選ぶ（表記ゆれ防止）。AI提案後は自動予測で上書きしない。
>
> **v1.0.552 の主な変更（2026-08-04）**（レシート読取後の入力画面を品目数で振り分け＝1品目は通常の支出フォームに統一）
>
> - ✨ **レシート読み取り後の入力画面を品目数で自動振り分け**（`receipt_ocr_flow.dart`）。
>   これまでは常に「品目ごとに記録」（`ReceiptSplitScreen`）だったが、**1品目（合計だけ読めた
>   領収書・医療領収証なども含む）は手入力と同じ「支出を記録」フォーム（`ExpenseInputScreen`）**に
>   流し込むように変更。2品目以上のときだけ従来の「品目ごとに記録」を使う。
>   → Android/Windows で入力画面・項目が揃い、単票のレシートは手入力時と全く同じ画面になる。
>   店名・金額・日付・カテゴリ（大/小）・レシート画像リンクはプリフィル。取引内容は品目名
>   （店名と重複する場合は空）を初期表示。
>
> **v1.0.551 の主な変更（2026-08-04）**（医療機関の領収証も読取／固定費の終了月保存バグ修正＋アーカイブ／固定費内訳もタップで一覧へ）
>
> - ✨ **レシート読み取りが医療機関の領収証・各種領収書に対応**（`receipt_ocr_cloud.dart`）。
>   Geminiのプロンプトを店舗レシート専用から拡張し、**医療機関の領収証・調剤薬局・タクシー・
>   サービスの領収書**も読めるように。医療領収証では金額に「今回領収額／領収額」(円)を使い、
>   **「点」(保険点数)は金額として絶対に使わない**よう明示。日付は「2026年08月03日」等の
>   和風表記や診療日/発行日も正規化。品目が無い領収書は items 空でOKに。
> - 🐛 **固定費編集で「終了月」を入れても保存できないバグを修正**（`subscription_list_screen.dart`）。
>   原因は「場所」が必須になっていたこと（場所は本来 任意の集計タグ）。**必須を「名前＋金額>0」だけに変更**し、
>   場所が空でも保存可に。場所未設定の既存固定費も編集・保存できるようになった。ラベルも「場所（任意）」に。
> - ✨ **固定費一覧に「アーカイブ」表示を追加**（`subscription_list_screen.dart`）。
>   右上のアーカイブアイコンで **現行 ⇄ 終了済み（終了月を過ぎたもの）** を切替。現行表示は
>   終了済みを自動的に隠してスッキリ。終了済み件数をバッジ表示。手動並び替えは絞り込み後の
>   並びでも崩れないよう ID ベースで反映（非表示分の位置は保持）。
> - ✨ **ホーム「支払方法別の内訳」の『固定費・サブスク』行もタップで固定費一覧へ**（`rich_home.dart`）。
>   これまで唯一詳細に飛べなかった行をタップ可能にし、固定費一覧（`SubscriptionListScreen`）を開く。
>
> **v1.0.550 の主な変更（2026-08-04）**（ホームの「支払方法別の内訳」をタップで詳細へ＋ロゴ表示／ヒーローは支出額を主役に）
>
> - ✨ **ホームの「支払方法別の内訳」の各行をタップで、その口座／カードの詳細画面へ遷移**
>   （`rich_home.dart`）。登録済みのカードは `CardDetailScreen`、銀行等は `AccountDetailScreen`
>   （通帳）へ。登録済み行の右端に「>」を出し、タップ可能なことを示す。未登録の支払方法
>   （固定費・サブスク等）はタップ不可のまま。詳細画面から戻ると自動で再読み込み。
> - ✨ **各行に支払方法のブランドロゴ（アイコン）を表示**（`rich_home.dart` + `BrandLogo`）。
>   登録済みで `iconUrl` があればロゴ画像、無ければウォレットアイコンにフォールバック。
>   従来の小さな色ドットを 24px のロゴ枠に置き換え。
> - 🎨 **ホームのヒーローカードを「支出額を主役」に変更（事業・個人 共通）**（`rich_home.dart`）。
>   これまで大きく出していた「今月の収支（差引）」をやめ、**大きい数字＝今月の支出額**（事業は経費）に。
>   収入・収支は下段のサブタイルに移し、収支タイルは黒字＝緑／赤字＝赤で色分け＋「黒字/赤字」バッジ。
>   数字を 34px に拡大し、収入は緑で見やすさを改善。
>
> **v1.0.549 の主な変更（2026-08-04）**（通帳の上部サマリーもスマホ最適化／明細タイトルを2行表示／クレカのカテゴリを色付きバッジに）
>
> - 🐛 **口座通帳の上部サマリー（月初残高/月末残高）がスマホで数字が潰れるのを修正**
>   （`account_detail_screen.dart`）。従来は PC 向けの横一列（月初｜収支｜月末）を
>   そのままスマホに出していたため、「¥219,050」「¥135,687」が **1文字ずつ縦に折れて**いた。
>   幅 <900px（スマホ）用に **`_balanceFlowStripMobile`** を新設：月初→月末を上段に大きく
>   （数字は `FittedBox` で自動縮小＝二度と縦折れしない）、実収入/実支出/振替を下段に色チップ3枚、
>   差引バッジを最下段に配置。PC幅（≥900px）は従来の横一列のまま。
> - 🐛 **スマホ明細のタイトルが「…」で切れて読めないのを改善**（`account_detail_screen.dart`）。
>   摘要を `maxLines:1`→**`maxLines:2`** にして、長い名称も折り返して見えるようにした。
> - ✨ **クレカ明細のカテゴリを色付きバッジ表示に**（`card_detail_screen.dart`）。
>   従来のグレー文字をやめ、`CategoryColors.effective()`（大カテゴリの色）に連動した
>   **色ドット＋淡色背景のバッジ**にして一目で分類が分かるようにした。説明文も2行まで表示。
>
> **v1.0.548 の主な変更（2026-08-04）**（口座通帳のスマホ表示崩れ修正／クレカ月切替を横スライドに）
>
> - 🐛 **口座通帳（入出金履歴）がスマホで崩れるのを修正**（`account_detail_screen.dart`）。
>   PC向けの幅広な固定列 Table（合計約642px）を狭いスマホ画面に押し込んでいたため、
>   「摘要」や「月末残高／月初残高」「¥314,783」などが**1文字ずつ縦に折れて崩れて**いた。
>   幅 <900px（スマホ）では **1明細＝1行のコンパクトなリスト表示**に切り替え（摘要はellipsis、
>   残高は下段のサブ行へ逃がす）。PC幅（≥900px）は従来の列そろえ Table のまま。
>   月初/月末残高は色付きバナー行にして、鉛筆から手入力編集も従来どおり可能。
> - ✨ **クレカ詳細の月切替をプルダウン→横スライド（‹ 2026年5月 ›）に**（`card_detail_screen.dart`）。
>   銀行通帳と同じ `MonthNavBar`（左右矢印）に統一。全期間⇄月別はアイコンボタンで切替、
>   期間指定（範囲）はそのまま。**Win/Android 共通**。
> - ✨ **スマホは左右スワイプでも月を切り替え可能に**（クレカ詳細・口座通帳の両方）。
>   クレカ詳細はタブ横スワイプを無効化し（タブはタップで切替）、明細リストの左右スワイプを
>   「月の切替」に割り当て。左スワイプ＝次の月／右スワイプ＝前の月。
>
> **v1.0.547 の主な変更（2026-08-02）**（Windows版の起動高速化・デスクトップ限定）
>
> - ⚡ **Windowsデスクトップ版の起動を速くした**（`desktop/main.js`）。ローカル配信サーバが
>   キャッシュヘッダを一切返しておらず、**起動のたびに `main.dart.js`(5.7MB) と
>   `canvaskit.wasm`(7.2MB) を毎回取り直し・再コンパイル**していた。`ETag`/`Last-Modified`＋
>   `Cache-Control: no-cache` と 304 応答を実装し、Chromium が本文と V8/WASM のコンパイル結果を
>   再利用できるようにした（実測：canvaskit.wasm の読込が毎回13MB転送→304で26msに）。
>   ファイル更新時は mtime/size で ETag が変わるため、**アプリ更新で古い画面が出る事故は起きない**。
>   ※この変更は Electron ラッパー限定で、Web/Android の中身は v1.0.546 と同一。
>
> **v1.0.546 の主な変更（2026-08-02）**（経費明細一覧の見た目）
>
> - 🎨 **日付を1行固定**（`date_weekday_text.dart`）。狭い日付列で「7/28(火)」の
>   閉じ括弧が次行へ折り返していたのを、`maxLines:1 / softWrap:false` ＋ 日付列幅を
>   52→58px に広げて解消。
> - 🎨 **支払方法チップを「登録済みロゴのみ」表示に**（`expense_list_screen.dart`）。
>   支払方法名が登録カード/口座名と一致し `iconUrl`（ロゴ）があれば、汎用アイコン＋名称の
>   代わりに **`BrandLogo` のロゴ画像だけ**を出す（三井住友カード等）。ロゴ未登録の支払方法は
>   従来どおりアイコン＋名称チップにフォールバック。`SettingsRepository.loadPayments()` を
>   読み、`PaymentsChangeNotifier` でロゴ変更に追従。
>
> **v1.0.532 の主な変更（2026-07-16）**（領収書ビューアのスクロール／まとめ行の見た目）
>
> - 🐛 **複数ページのPDF領収書をホイールで送れなかったのを修正**（`receipt_viewer_screen.dart`）。
>   `PdfViewerParams.scrollByMouseWheel` を **`null`（＝ホイールがズームになる）にしていた**のが原因。
>   `0.5` にして**ホイール＝スクロール**に戻した。GUの注文明細（5ページ）で発覚。
>   - ズームは **AppBarの拡大/縮小ボタン**（`PdfViewerController.zoomUp/zoomDown`）・ピンチ・
>     Ctrl+「＋/−」で行う。⚠ **ここを再び null にしないこと**（ページ送りが死ぬ）。
> - 🐛 **まとめ明細画面の「領収書を見る」でPDFが開けなかったのを修正**。
>   `ReceiptImageScreen`（`Image.memory`＝画像専用）を開いていたため。明細詳細と同じ
>   `ReceiptViewerScreen`（PDF/画像を自動判定・失敗時はDriveで開く）に統一した。
> - まとめ行の**開閉アイコン（▶）を削除**。「N品」バッジだけでレシート束だと分かるため。
>
> **v1.0.531 の主な変更（2026-07-16）**（レシートまとめ行は展開せず詳細画面へ）→ 詳細は 6.5
>
> - 明細テーブルの「N品」まとめ行を**タップしても表の中で展開しない**。
>   代わりに **`ReceiptGroupDetailScreen`（まとめ明細）を開いて品目を見せる**。
>   表が縦に伸びて他の明細が下へ押し出されるのを避けるため。
> - `ExpenseDetailTable` に `onOpenGroup`（必須）を追加。呼び出し側（支出タブ
>   `rich_expenses` / クレカ詳細 `card_detail_screen`）が画面を開き、変更されたら再読込する。
>   `expense_list_screen` は以前から同じ画面へ飛んでいたので**挙動が揃った**。
>
> **v1.0.530 の主な変更（2026-07-15）**（レシート保存フォルダを移動しても追従する）
>
> - 📁 **レシートのDrive保存先＝`マイドライブ/④個人事業/FutaFinanceレシート`**
>   （2026-07-15にユーザーがマイドライブ直下から移動。アカウントは contact@＝H:ドライブ）。
>   配下は `(事業用|個人用)/YYYY年/MM月/`。現状 `個人用` のみ実在。
> - 🐛 **移動で顕在化する分裂バグを修正**（`drive_receipt_service.dart`）。
>   `_ensureRoot` / `_findMonthFolderOnly` はルートフォルダIDが prefs に無いとき
>   `FutaFinanceレシート` を**マイドライブ直下だけ**（`'root' in parents`）探しており、
>   移動後は見つけられず**直下に空の同名フォルダを新規作成**＝保存先が2箇所に分裂していた。
>   - 対策＝`_findRootAnywhere()`＝**親を問わずドライブ全体から名前で探す**。
>     見つからないときだけ直下に作る。
>   - ⚠ **ルートフォルダの場所を決め打ちしないこと。** 一度使った端末は prefs の
>     `futa.drive.receipt_root_id` でIDが効くため**バグに気づけない**。新しい端末・
>     再インストール・prefsクリアのときだけ発動する。
>
> **v1.0.529 の主な変更（2026-07-15）**（未計上の固定費を支出明細に出さない）
>
> - **請求日がまだ来ていない固定費は支出明細に出さない**（`rich_expenses.dart`
>   `_fixedTableRows`）。`date.isAfter(today)` の行を捨てる。
>   明細は「もう出て行ったお金」を突き合わせる場所なので、未計上の予定が混ざると邪魔になる。
>   - 過去月を見ているときは全部過去日なので、この判定は効かない（従来どおり全部出る）。
>   - `_fixedTableRows` は `ExpenseDetailTable` の `fixedRows` にしか渡していない
>     ＝**表示だけ。支出合計・カテゴリ別・支払方法別・毎月の固定費の集計は不変。**
>   - ⚠ **クレカ明細（`card_detail_screen._cardFixedRows`）は対象外**。あちらは
>     「今月このカードにいくら請求されるか」を見る画面なので予定も出す。
>
> **v1.0.528 の主な変更（2026-07-15）**（明細テーブルのレシートまとめ行）→ 詳細は 6.5
>
> **v1.0.527 の主な変更（2026-07-15）**（レシート記録を「品目ごと」に一本化）→ 詳細は 6.4
>
...
>
> - **固定費編集の「場所」を必須に**（`subscription_list_screen.dart` `_editDialog`）。
>   `isValid` に `storeCtrl.text.trim().isNotEmpty` を追加＝**空だと保存ボタンが押せない**。
>   ラベルも「場所（任意）」→「場所」。`onChanged` で `setLocal` して活性を切り替える。
> - **大カテゴリを選んだとき、小カテゴリを自動で入れないようにした**。
>   以前は `categorySub = subs.isNotEmpty ? subs.first : null` で**先頭の小カテゴリを勝手に選んで**いた
>   （意図しない小カテゴリのまま保存される）。`categorySub = null` にして自分で選ばせる。
>
...
>
> - **取引詳細の「領収書」カードを3択チップに変更**（`transaction_detail_screen.dart`
>   `_receiptChoiceChip`）。**［URLを貼る］［紙で保管］［未保管］**。
>   - ⚠ 以前は「URLを貼るボタン」＋「紙のチェックボックス」で、**同じ1つの問い
>     （この領収書をどう保管したか）なのに聞き方が2種類**あり不揃いだった。
>     固定費編集の「領収書の受け取り方」（紙で保管／ドライブ／指定なし）と同じチップ形に統一。
>   - 3択なので必ずどれか1つに決まる（「ボタンとチェックが同時に立つ」曖昧さが消える）。
>   - 選択中は枠＋文字がアクセント色＋チェックアイコン。
> - **証憑があるときに「URLを直す」リンクを追加**（貼り間違い・自動取込の貼り替え用）。
>   以前は一度URLが付くと詳細画面から直せなかった。
>
> **v1.0.521 の主な変更（2026-07-15）**（クレカ明細も詳細画面を経由する）
>
> - **クレカ明細（`CardDetailScreen`）の鉛筆を、編集フォーム直開き → 明細の詳細画面
>   （`TransactionDetailScreen`）経由に変更**（`_editCardTxn`）。
>   支出タブ（`rich_expenses` の `_edit`）は元から詳細画面経由で、**ここだけ挙動が違っていた**
>   （同じ鉛筆アイコンなのに、支出タブ＝詳細／クレカ明細＝いきなり編集フォーム）。
>   ※モードの違いではなく画面の違い。事業/個人どちらも同じ挙動になる。
> - **例外**：クレカ棚卸し（突合シート `showCardReconcileSheet` の `onEditTxn`）は
>   「金額を直す」流れなので、従来どおり編集フォームを直接開く。
>
...
>
> - 🐛 **【重要】支払方法別の内訳が固定費を二重計上していたのを修正**（`rich_expenses.dart`）。
>   種類別の合計(`_subsOf`)は `_matchedSubIds` で「実明細として記録済みの固定費」を除外しているのに、
>   **支払方法別(`byPayment`)だけ全件足していた**。→ 同じ `_matchedSubIds` で除外するよう修正。
>   - 実例（2026年4月・個人）：支払方法別のオリコ 175,468円 に対しウォレットは 149,940円。
>     差 25,528円＝オリコ払いの固定費5件（あすけん480/Auns Gym 6,478/Google One 290/
>     ASPIジム 17,680/Amazon Prime 600）が、実明細と固定費予定で二重に足されていた。**ウォレット側が正しい**。
> - **高額明細の行をタップで、その明細の編集を開けるようにした**（`_summaryLine` に `onTap` を追加）。
> - **銀行明細の詳細に「場所」行を追加**（`transaction_detail_screen.dart`・入っている時だけ表示）。
> - **振替フォーム**（`transfer_input_screen.dart`）:
>   - **「名前」を一番上（金額の上）に移動し、必須に**した。自動命名（`移動元 → 移動先`）を廃止。
>     明細で見たとき「オリコ先払い」のように意味が分かる名前を必ず付けてもらう。
>   - **出し方を支出/収入と同じ `showInputSheet` に統一**（PC=中央のコンパクトなダイアログ／
>     スマホ=下からのシート）。以前はここだけ「画面の95%の高さのシート」で、
>     「支出を編集」→「振替に変更」で見た目がガラッと変わっていた。ヘッダー（タイトル/削除アイコン）も統一。
> - **Electron標準メニューを廃止**（`desktop/main.js`・`Menu.setApplicationMenu(null)`）。
>   `autoHideMenuBar` だけでは Alt/Ctrl 操作でメニューバーが顔を出していた。
> - **個人モードの2026年1月より前のデータを削除**（1件・2025-12-29 の振替「オリコカード 引落（11月利用分）」
>   25,528円）。表示上は `AppMode.personal.minDate`(=2026-01-01) で元から出ていなかった。
>
> **v1.0.516 の主な変更（2026-07-15）**（個人の業績タブ：税金・保険料の表／月別収支グラフを廃止）
>
> - **個人モードの業績タブに「税金・保険料」セクションを追加**（`v2_report.dart` `_taxTableCard`）。
>   **行＝税目・列＝1〜12月＋年間合計**の月次表（PLと同じ `_PLTable`/`_PLRow` を再利用・横スクロール）。
>   - 対象＝**大カテゴリ**（番号を剥がした素名）が `_taxMajorKeywords`
>     （`税金`/`租税公課`/`保険`/`年金`/`社会保険`/`健康保険`/`法定福利費`）を含む支出。
>     ⚠️ **小カテゴリ・摘要は見ない**。実データに「健康サプリ」「健康投資」があり、
>     摘要まで見ると税金と無関係の項目を拾ってしまうため（実データで確認済み）。
>   - 行ラベル＝**小カテゴリ**（所得税・住民税など）。小カテゴリが空なら大カテゴリの素名。
>   - 金額は `effectiveAmount`（立替回収分を引いた実質負担）。年間合計の降順＋最終行に「合計」。
>   - 対象の記録が無い年は**セクションごと非表示**（`_taxTableCard` が null を返す）。
>   - 実データ例（2026年）：所得税 950,749円 / 住民税 242円。保険は個人カテゴリに未登録のため
>     行は出ない（カテゴリを作れば自動で並ぶ）。
> - **「月別の収支（収入 − 支出）」の棒グラフを廃止**（個人モードのみのセクションだった）。
>   年間の収支・貯蓄率は上の「◯年のサマリー」で分かるため。
>
> **v1.0.515 の主な変更（2026-07-15）**（支出サマリーに高額明細）
>
> - **支出タブの「◯月の支出合計」→「内訳を見る」に「高額明細（1万円以上）」を追加**（`rich_expenses.dart`）。
>   種類別・支払方法別の下に、**実質負担が `_kBigAmount`(=10,000円) 以上の取引を金額降順**で
>   「日付＋取引内容＋金額」の1行ずつ並べる。0件なら見出しごと出さない。
>   - 判定は `effectiveAmount`（立替回収分を引いた実質負担）＝立替で戻る分は高額扱いしない。
>   - 対象は取引のみ（固定費・サブスクは「種類別」に合算で出るので混ぜない）。
>   - 狙い：カテゴリ内訳を1つずつ開かなくても「今月なににお金が飛んだか」が一目で分かる。
> - **「振替に変更」ボタンを目立たなくした**（`expense_input_screen.dart`）。
>   色付きの大きなチップ＋左寄せだと入力欄より先に目に入るため、**小さいグレーの文字リンク・右寄せ**に変更
>   （めったに使わない操作のため）。
>
> **v1.0.514 の主な変更（2026-07-15）**（明細URLを後から貼れる・スクロールバーの当たり）
>
> - **取引詳細の「領収書」カードに「◯◯のURLを貼る」ボタンを追加**（`transaction_detail_screen.dart`）。
>   証憑が未設定のとき、これまでは「紙のレシートで保管済み」チェックしか無く、**あとから明細URLを
>   付けられなかった**（`ExpenseInputScreen` の編集画面まで開く必要があった）。
>   - ダイアログにURLを貼ると `receiptUrl` に保存し、**`receiptSaved=true` / `receiptType='drive'` も同時にON**。
>     ＝**「明細URL」でも「紙フラグ」でも、どちらか立てば対応済みチェックがONになる**（紙は `_setPaperKept`）。
>   - 保存後はカードが「📄 ドライブに保管」＋「◯◯を見る」に切り替わる。事業モードのみ（従来どおり）。
>   - 携帯の利用明細ページのように、Drive以外のURLも貼れる。
> - **明細表の右にスクロールバー用の余白（14px）を追加**（`expense_detail_table.dart` の `stickyHeader` 時）。
>   クレカ明細でスクロールバーが行末の編集（鉛筆）ボタンに重なって押しにくかったのを解消。
>
> **v1.0.513 の主な変更（2026-07-15）**（支出を後から振替に変更できる）
>
> - **「支出を編集」に「この取引を振替に変更」を追加**（`expense_input_screen.dart`）。
>   ATMの現金出金のように、支出として記録してしまった口座間の移動を、あとから振替へ作り替えられる。
>   - 押すと**同じ取引ID**のまま `TransferInputScreen` を「振替に変更」モードで開く
>     （`showTransferInputModal(editing: 支出)`）。移動元/先を選んで保存すると `type` が
>     `expense` → `transfer` に変わり、収支（PL）から外れて口座残高の移動だけになる。
>   - **移動元は支払方法（例: 住信SBI）を初期値**にプリフィル。取引内容は「名前」に引き継ぐ。
>   - 変更時に**証憑リンク（`receiptUrl`/`receiptId`/`receiptSaved`/`receiptType`）と場所を保持**する
>     （振替画面には出ないが、間違えて変えたときに失われないように）。
>   - 立替精算の回収用振替（`settle_<取引ID>`）が紐づいていれば、変更後に**自動で削除**（二重計上防止）。
> - `TransferInputScreen`：**候補に無い口座名が入っていたら未選択に落とす**（`_load`）。
>   休眠にした/改名した口座、支払方法がクレカだった支出からの変更で Dropdown が値を見つけられず
>   画面が落ちるのを防ぐ。
> - ※逆方向（振替→支出）は従来どおり**通帳の種別ポップアップ**（`_typeCell`）で変更する。
>
> **v1.0.512 の主な変更（2026-07-11）**（銀行明細専用の詳細画面）
>
> - **登録銀行口座に紐づく取引は「銀行明細」専用レイアウトで詳細表示**（`transaction_detail_screen.dart`）。
>   経費の型（カテゴリ／領収書保管／立替）を隠し、**口座・入金/出金/振替・摘要・日付**だけを通帳の1行として見せる。
>   - 対象＝支払方法(`paymentMethod`)が登録銀行口座名と一致する支出/収入、または振替の from/to が登録銀行口座。
>     現金・電子マネー・クレカ・未登録先は従来どおり汎用画面。判定は起動時に `SettingsRepository.loadPayments()` の
>     `bankAccounts` を読んで口座名で突合（`AccountType.bank` のみ）。
>   - 種別バッジは **入金／出金／振替** の3種のみ（振込手数料も「出金」に含める）。振替は「元口座 → 先口座」を表示。
>   - **「◯◯の通帳を開く」ボタン**を追加（該当口座の `AccountDetailScreen` へ。通帳側で編集/削除したら一覧を再読込）。
>   - **例外**：収入（売上入金）で `receiptUrl` がある場合だけ「請求書を見る」リンクを残す。
>   - 残高の逆算・「この取引の後の残高」は出さない（口座＋入出金だけの軽い作り）。
>
> **v1.0.511 の主な変更（2026-07-11）**（固定費・クレカ引落の自動記録を内訳ポップアップ化）
>
> - 固定費（サブスク）の実明細化と、クレカ自動引落の自動記録の**お知らせをスナックバー→ポップアップに変更**（`v2_root.dart`）。
>   従来は「固定費をN件、実明細として記録しました」とだけ出て**何がいくら記録されたか分からなかった**。
>   共通ヘルパー `_showMaterializedDialog` を追加し、**日付(M/D)・取引内容・金額の一覧＋合計**をダイアログで表示する。
>   固定費＝🧾／クレカ引落＝💳。初回のみ通知する `_unnotifiedIds` の仕組みは従来どおり（毎回は出ない）。
>
> **v1.0.510 の主な変更（2026-07-11）**（個人モードでも領収書を閲覧可能に）
>
> - **個人モードの明細画面に「領収書を見る」ボタンを追加**（`transaction_detail_screen.dart`）。
>   個人モードは税務用の保管トグル（紙で保管済み等）は不要なので出さないが、Amazon等で
>   Driveに領収書が紐づく取引（`receiptUrl` あり）では閲覧ボタンだけ表示する。
>   条件 `if (!isBusiness && hasReceipt)`。閲覧は既存の `ReceiptViewerScreen`（恒久対応済）を再利用。
>
> **v1.0.508 の主な変更（2026-07-10）**（事業モードに「税務顧問料を除く」トグル）
>
> - **事業モードの経費タブに「税務顧問料を除く」トグルを追加**（個人モードの「家賃を除く」と同型）。
>   顧問料は毎月ほぼ固定の大きめ費用で他の経費の増減が霞むため、ワンタップで表示/非表示を切替。
> - 実装：`UiPreferences.hideAdvisory`（`futa.ui.hide_advisory`）＋`rich_expenses.dart`の
>   `_advisoryKeywords=['税務顧問料','顧問料','税理士']`／`_isAdvisoryTx`・`_isAdvisorySub`／
>   `_advisoryToggleChip()`（経費見出し行の左）。**取引・固定費の両方**を除外し、`_visibleSubs`にも反映。
> - **締めスナップショットは実額（顧問料込み）を記録**（`keihiFullTotal`＝隠していても正しい額を残す）。
>   諸経費タブの表示合計は隠した分を反映（`keihiTotal`）。
>
> **v1.0.507 の主な変更（2026-07-10）**（証憑ビューアの404恒久対応）
>
> - 領収書がアプリ内で404になる不具合を、デスクトップはElectronメインプロセスDLで根治（§証憑ビューア参照）。
>
> **v1.0.505 の主な変更（2026-07-08）**（家賃振込の除外）
>
> - **家賃除外キーワードに「ナカネ」を追加**（`_rentKeywords`）。共同生活費（家賃）は
>   銀行振込で支払われ摘要が「振込 ナカネ ハルカ」となるため、"家賃"も"共同生活費"も
>   含まず拾えていなかった。振込先名義「ナカネ」を家賃扱いにして「家賃を除く」で拾う。
> - 補足：銀行CSV取込は「出金→支出／入金→収入、振替ONのみ収支外」。他人・他社あて振込を
>   振替にしたい場合は通帳の種別ポップアップ（v499）で個別変更する（自動化は未実装）。
>
> **v1.0.504 の主な変更（2026-07-08）**（家賃除外の判定拡張）
>
> - **「家賃を除く」の判定に「共同生活費」を追加**（家賃＝共同生活費として運用のため）。
>   キーワード＝`['家賃','共同生活費','共同生活']`（`_rentKeywords`・`_matchesRent`）。
> - 判定対象を**摘要（description）にも拡張**：取引は 大／小カテゴリ＋摘要、
>   固定費は 名称／カテゴリ／会計科目 のいずれかにキーワードを含めば除外対象。
>   （従来は「家賃」のみ・カテゴリのみ判定だったため、名前が「共同生活費」の項目が
>   除外されなかった不具合を修正）。
>
> **v1.0.503 の主な変更（2026-07-08）**（立替の表記＆編集時の現金回収）
>
> - **明細の立替表記を「(実際支払 ¥X)」**に変更（v502の「実質支払」から）。
> - **編集での立替化でも「もらう現金」を受け取り先ウォレットに加算**するようにした
>   （新規と同じ挙動に統一・`_reconcileSettleForEdit`）。
>   - 現金回収の振替IDを**決定的な `settle_<取引ID>`** にし、再編集で同じ振替を追える。
>   - **二重作成を防ぐ冪等再構成**：保存のたびに既存の振替を基準に upsert／削除し、
>     受け取りウォレット残高キャッシュも「旧を戻す→新を足す」で差分反映。
>   - 立替OFF／もらう現金0にすると、振替を削除し残高も巻き戻す。
>   - 旧仕様（タイムスタンプID）で作られた過去の立替も、From「立替精算」＋メモの支出名で
>     見つけて引き継ぐ（過去分の編集でも二重にならない）。
>
> **v1.0.502 の主な変更（2026-07-08）**（立替の表記＆後付け立替）
>
> - **明細の立替表記を「(実質支払 ¥X)」＋半角カッコ**に変更（`_txAmountCell`）。
>   主＝実質負担額、副＝`(実質支払 合計額)`。
> - **登録済みの支出を、後から立替精算に変えられる**ようにした（`expense_input_screen.dart`）。
>   - 編集画面にも「立替精算」トグルを表示。ONにして**自分の負担額**を入れると、
>     `reimbursed = 満額 − 自己負担` を記録し、明細・集計に実質負担が反映される。
>   - 編集モードは**現金の受け取り先ウォレット選択・現金回収の振替は作らない**
>     （実質の負担を記録するだけ。再編集で振替が二重に増えるのを防ぐ）。
>   - 既存の立替支出を編集で開くと、トグルON＋自己負担額がプリフィルされる。OFFで立替解除。
>
> **v1.0.501 の主な変更（2026-07-08）**（支出明細の金額表記）
>
> - **立替精算した支出は、明細の金額セルを「実質負担額（合計支払額）」の2段表示**にした
>   （`_txAmountCell`・`expense_detail_table.dart`）。例：`-¥4,000` ＋ 小さく `（合計 ¥9,800）`。
>   - 対象：`reimbursed > 0` の支出行のみ。立替なしの行は従来どおり満額を1行表示。
>   - PC版（`_ExpenseRow`）・スマホ版（`_NarrowRow`）・並び替えリストの3箇所に適用。固定費行は対象外。
>   - 合計額（満額）はカード明細と一致させるため保持。集計・一番上の合計は従来どおり実質負担（`effectiveAmount`）。
>
...
>
> _場所マスタ・入力・UI_
> - 場所マスタに **検索バー**（名前で絞り込み・件数が多くても目当てを探せる）と、
>   各場所を**タップして紐づく明細一覧をボトムシート表示**（新しい順・件数/合計つき）を追加（v1.0.545）。
...
>
> _不具合・パフォーマンス_
> - **【v1.0.534】スマホの支出明細から領収書・検収のチェックボックスを外した**：
>   締め処理（検収）は必ずPCで行うため、狭い画面ではチェックが場所を食うだけで使わない。
>   狭い幅（`narrow`）の `_NarrowRow`/`_NarrowFixedRow` に `showReceipt: false`/`showReview: false`
>   を渡す（`_NarrowGroupRow` は元からチェック無し）。PC（広い幅）とデスクトップは従来どおり。
> - **【v1.0.533】スマホで支出明細が丸ごと灰色（ErrorWidget）になる不具合を修正**：
>   `_NarrowSortBar.build` が `_SortCol.values` を全部回して `_labels[c]!` を引いていたが、
>   `_labels` に **`custom`（カスタム並び替え）のラベルが無く**、`null!` で毎回クラッシュしていた
>   （v1.0.418で `custom` を列挙に足したとき混入した潜在バグ。PCの表は別コードなので無事だった）。
>   `custom` はタップして選ぶソート列ではないので、**ラベルの有る列だけチップにする**
>   （`if (_labels.containsKey(c))`）。回帰テスト `test/narrow_table_repro_test.dart` で
>   狭い幅の描画に例外が出ないことを保証。⚠️ **列挙に値を足したらラベル表も必ず更新する**。
...
| ドキュメント | 型 | 用途 |
|---|---|---|
| ~~`config/{business,personal}_month_closing`~~ | ~~MonthClosingConfig~~ | **v1.0.555で撤去・データ削除済**（締め機能廃止） |
| ~~`config/{business,personal}_checklist`~~ | ~~ChecklistConfig~~ | **v1.0.555で撤去・データ削除済**（確認チェックリスト廃止） |
...
### 6.5 明細テーブルのレシートまとめ行（v1.0.528〜）

`ExpenseDetailTable`（支出タブ・クレカ明細の共用テーブル）は、**同じ `receiptId` の
取引が2件以上あるとき1行に束ねて表示**する。「カード明細1行＝アプリ1行」にして
突き合わせできるようにするのが目的。

- 実装: `_Row.group(members)` ＋ `_groupByReceipt()`（`expense_detail_table.dart`）。
  **親レコードはDBに作らない。** 合計は描画のたびメンバーから算出するので、
  支出合計・カテゴリ別・支払方法別の集計は一切変わらない（二重計上しない）。
- **まとめ行タップ＝「まとめ明細」画面（`ReceiptGroupDetailScreen`）へ**（v1.0.531〜）。
  表の中では展開しない（旧 `_expanded` 方式は廃止）。表が縦に伸びて他の明細が
  押し出されるのを避けるため。開閉アイコンは置かない（「N品」バッジで分かるので・v1.0.532〜）。
  - 呼び出し側が `ExpenseDetailTable.onOpenGroup` で画面を開き、戻り値が `true`
    （編集/削除された）なら再読込する。
- 「内容」列＝店名＋「N品」バッジ。大/小カテゴリと支払方法はメンバーが全員同じなら
  その値、違えば「複数」。
- 領収書・確認チェックはまとめ行で**全品目に一括反映**（1枚のレシートなので）。
- 検索（絞り込み・ページ内検索）は品目名にもヒットする（`searchHay` に全品目を含む）。
- **並び替えドラッグ中（`customEditable` ON）は束をほどいて1品ずつ表示**する。
  並び順は品目ごとの `sortOrder` に保存するため、束のままだと書き戻せない。
- ホーム（`rich_home` / `v2_home_topnav`）と `expense_list_screen` には以前から
  同等のまとめ表示があった。v2の明細テーブルだけ未移植だったのを揃えた。
...
- **⚠️ デスクトップ(Electron)は公開DLもメインプロセスで行う（v1.0.507〜）**: v1.0.506の公開DLフォールバックはレンダラ(Flutter Web)の`http`=ブラウザfetchで叩いていたが、Electron内(`http://127.0.0.1:50873`配信)では**それでも404が続いた**（CORSは`ACAO:*`でOKなのにブラウザ側で取得できない事象）。→ **Electronメインプロセス(Node `https`)で公開DL URLから取得しbase64で返す**経路を追加＝ブラウザのfetch/CORSに一切依存せず確実。実装：main.js `downloadDriveFile`/IPC `futa:downloadFile`、preload `downloadFile`、`desktop_bridge*.desktopDownloadFile`、`_downloadPublic`が**デスクトップ時はまずメインプロセス経由**を試す。Web/Androidは従来の公開DL（そちらはfetchで問題なく取れる）。
