...
class ExpenseDetailTable extends StatefulWidget {
  ...
  /// レシートまとめ行（「N品」の行）タップ時。品目の内訳は表の中で展開せず、
  /// 「まとめ明細」の詳細画面で見せる（v1.0.531〜）。表が縦に伸びて他の明細が
  /// 押し出されるのを避けるため。呼び出し側で画面を開き、変更があれば再読込する。
  final Future<void> Function(List<core.Transaction> members) onOpenGroup;
              ...
              if (narrow) {
                return Column(
                  children: [
                    ...
                    _plainRows(
                      detailRows,
                      // ⚠️ スマホ幅は領収書チェックボックスを出さない（2026-07-18）。
                      //   狭い画面ではチェックが場所を食うだけで使わない。行タップは
                      //   編集/まとめ明細に使い、チェックはPC（広い幅）に任せる。
                      (r) => r.isFixed
                          ? _NarrowFixedRow(
                              f: r.fx!,
                          ...
                          : r.isGroup
                              ? _NarrowGroupRow(
                                  ...
                                  total: r.amount,
                                  title: r.contentText,
                                  onTap: () => widget.onOpenGroup(r.members!),
                                )
                              : _NarrowRow(
                                  t: r.txn!,
  ...
  Widget _customReorderList(List<_Row> groupedRows) {
    ...
    return ReorderableListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        // 通帳（銀行）のカスタム順と同じ「ハンドル＋日付＋内容＋金額＋編集」の1行。
        return DecoratedBox(
          key: ValueKey(isFixed ? 'fx_${r.fx!.id}' : 'tx_${r.txn!.id}'),
          decoration: BoxDecoration(
            // 固定費は淡いアンバー背景で区別。
            color: isFixed ? _kFixedBg : Colors.white,
            border:
...
const Color _kGroupAccent = Color(0xFF1D4ED8); // 「N品」バッジ文字
...
class _ExpenseRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        height: _kRowH,
        // 固定費=淡いオレンジ背景（それ以外は無し）。
        color: isFixedCat ? _kFixedBg : null,
        child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 12),
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.center,
...
/// 同じレシートの品目を束ねた1行（PC幅）。取引行と同じ列割りだが、淡いブルー
/// 背景＋「N品」バッジでレシート1枚分だと分かるようにし、タップすると品目の
/// 内訳を「まとめ明細」画面で開く。
      ...
      child: Container(
        ...
        child: Padding(
            ...
            child: Row(
              ...
              children: [
                SizedBox(
                    width: w.date, child: _DateWithWeekday(date: _head.date)),
                _vGrid(_kColGap, _kRowH),
                SizedBox(
                  width: w.major,
                  child: Align(
...
/// 同じレシートの品目を束ねた1行（スマホ幅）。タップで「まとめ明細」画面へ。
