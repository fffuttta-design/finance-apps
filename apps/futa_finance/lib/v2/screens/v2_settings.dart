...
class _V2SettingsScreenState extends State<V2SettingsScreen> {
  ...
  static const _menus = <_MenuGroup>[
    ...
    _MenuGroup(title: 'データ管理', items: [
      _MenuItem('backup', 'バックアップ / 取り込み',
          ...
          desc: 'データの書き出し・取り込み'),
      _MenuItem('aiUsage', 'AI使用量', Icons.query_stats,
          desc: 'Claude・ChatGPTをどのツールがいくら使ったか'),
    ]),
  ...
  Widget _buildPanelInner() {
    switch (_currentId) {
      ...
      case 'storeMaster':
        return _embedV1(const StoreMasterScreen(),
            title: '場所マスタ',
      ...
      case 'aiUsage':
        return _embedV1(const AiUsageScreen(),
            title: 'AI使用量',
            note: 'Claude・ChatGPT を どのツールが いくら使ったか。金額はトークン数×公式単価の概算で、'
                'クレジット購入額はカードの実額です（公式の使用量APIは個人アカウントでは'
