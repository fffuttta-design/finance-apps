...
class _V2ReportScreenState extends State<V2ReportScreen>
  ...
  // ── 新デザインのダッシュボード帯用：月次の売上/経費（固定費込み）──
  int _bandIncome(DateTime m) => _transactions
      .where((t) =>
  ...
  int _bandExpense(DateTime m) {
    final tx = _transactions
        ...
        .fold<int>(0, (s, t) => s + t.amount);
    final now = DateTime.now();
    final nowYm = '${now.year}-${now.month.toString().padLeft(2, '0')}';
    final ym = '${m.year}-${m.month.toString().padLeft(2, '0')}';
    final subs =
        _subs.fold<int>(0, (s, sub) => s + sub.plAmountForMonth(ym, nowYm));
    return tx + subs;
  }

  /// 上部ダッシュボード帯（月別の収支推移）。richBand のときだけ表示。
  Widget _richTrendBand() {
    final months = <DateTime>[
      for (int i = 5; i >= 0; i--)
        DateTime(_selMonth.year, _selMonth.month - i),
    ];
    final data = [
      for (final m in months)
        (month: m, income: _bandIncome(m), expense: _bandExpense(m)),
    ];
    final maxVal = data.fold<int>(1, (mx, e) {
    ...
    Widget bar(int v, Color c) {
      final h = (v / maxVal).clamp(0.0, 1.0) * chartH;
      return Container(
        width: 11,
        height: h < 3 ? 3 : h,
        decoration: BoxDecoration(
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              legendDot(V2Colors.positive, '売上'),
              const SizedBox(width: 12),
              legendDot(V2Colors.negative, '経費'),
            ],
          ),
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: Row(
                            ...
                            crossAxisAlignment: CrossAxisAlignment.end,
                            children: [
                              bar(e.income, V2Colors.positive),
                              const SizedBox(width: 4),
                              bar(e.expense, V2Colors.negative),
                            ],
                          ),
                        ),
